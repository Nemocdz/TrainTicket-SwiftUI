...
class TicketLayoutAttributes: UICollectionViewLayoutAttributes {
    var isExpand = false

    override func copy(with zone: NSZone? = nil) -> Any {
        let attribute = super.copy(with: zone) as! TicketLayoutAttributes
        attribute.isExpand = isExpand
        return attribute
    }
}

public class CustonTicketLayout: UICollectionViewLayout {
    fileprivate var insertPath = [IndexPath]()
    fileprivate var deletePath = [IndexPath]()
    fileprivate var attributeList = [TicketLayoutAttributes]()
    ...
    fileprivate func generateAttributeList() -> [TicketLayoutAttributes] {

        var arr = [TicketLayoutAttributes]()
        ...
        for sec in 0..<sections {
            ...
            for item in 0..<count {
                if itemsIdx >= startIdx {
                    ...
                    let attr = TicketLayoutAttributes(forCellWith: indexPath)
    ...
    fileprivate func setNoSelect(attribute: TicketLayoutAttributes) {
    ...
    fileprivate func setSelect(attribute: TicketLayoutAttributes) {
    ...
    fileprivate func setBottom(attribute: TicketLayoutAttributes, bottomIdx:inout CGFloat) {
